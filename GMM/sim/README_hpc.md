...
1. Simulate one dataset and draw `n_starts` initial vectors once.
...
- `start_lower`, `start_upper`: 14-element absolute uniform bounds. The example
  is true beta +/- 2. Starts are drawn once; they are not redrawn inside a path.
  Omitting both bounds uses `true_beta + rnorm(14, 0, start_sd)`.
- `n_starts`: number of independent Stage 1 paths, default 20.
- `loops`: maximum beta-W iterations **per Stage 1 path**, default 6.
...
worker output files also stop aggregation rather than silently dropping jobs.
...
and path convergence. The directory's `loop_01` is a historical label for the
...
- `.diagnostics.rds`: settings, initial vectors, `starts` (full independent
  ...
  path convergence), `top_start_ids`, `stage1`, `refined` (original objective,
