...
- Loop 1 submits `STARTS` independent perturbed start points and waits for
  `TARGET` converged results. The defaults are 50 starts and 20 converged
  results.
...
To submit seeds 1 through 10 with the default `J=200`, 50 loop-1 starts, and
advance after 20 converged results:
...
only loop 1: the number of random starts and the number of rows needed to move
to loop 2. With the default `--target-type converged`, the target is the number
of converged rows. Later loops always use one start. The next beta is chosen
only from `convergence == 0` rows unless `REQUIRE_CONVERGED_BEST=0` is set for
debugging. If `--tol` is positive, the driver stops early when
`max(abs(beta_new - beta_old)) <= tol`.

The driver stops with an error if an array job finishes before its required
target is reached. This prevents a loop from advancing with fewer than the
requested converged results.
...
writes the best converged beta from each loop to:
