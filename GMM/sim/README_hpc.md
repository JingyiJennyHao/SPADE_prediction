...
For a small HPC plumbing smoke test, submit 10 starts and move to the next loop after one completed finite result:

```bash
STARTS=10 TARGET=1 LOOPS=3 REQUIRE_CONVERGED_BEST=0 OUT_DIR=hpc_runs/smoke_seed1_10 ./run_loop.sh
```

`REQUIRE_CONVERGED_BEST=0` is intended for debugging the submit/wait/resume procedure. For the usual simulation, keep the default `REQUIRE_CONVERGED_BEST=1`.
