...
run_one_sim <- function(seed, out_csv) {
  ...
  W_current <- diag(3*14)
  stpt0 <- c(2,0.5,0.5, 1,1,1, 0.5, 0.3,0.3,0.3, 0.2,0.2,0.2, 6)
  beta_old <- stpt0
  
  for (loop in 1:1) {
    print(loop)
    # β-step:
    optimal_list <- vector("list", 1)
    
    for (rep in 1:1) {
      stpt <- beta_old + rnorm(14,0,0.1)
      #stpt <- c(runif(13, min=0, max=1), 6)
      fit <- optim(stpt, gmm_obj, sim_dat = sim_dat, W_hat = W_current, Time = 3, control = list(maxit = 5000))
    ...
    beta_new <- converged[[pick]]$par
    
    diff_beta <- max(abs(beta_new - beta_old))
    cat("iter =", loop,"diff =", diff_beta,"\n")

    ## update beta and weight
    beta_old <- beta_new
    W_current <- W_hat_fun(beta_old, sim_dat, Time = 3)
    if(diff_beta < 1e-5){break}
  }
...
```{r}
run_one_sim(1,out_csv)

args <- commandArgs(trailingOnly = TRUE)
# usage: Rscript run_one_sim.R <out_csv> <seed>
out_csv <- ifelse(length(args) >= 1, args[1], "sim_results__0501_200.csv")
