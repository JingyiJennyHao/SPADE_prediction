...
score_ll_row <- function(j, t, beta, sim_dat){
  ...
  score_jt <- numDeriv::grad(f_jt, beta)
  return(score_jt)  
}
...
## -------------------- One simulation --------------------
#%%
run_one_sim <- function(seed, out_csv, true_beta = gmm_default_beta,
                        ...
                        diagnostics_file = paste0(out_csv, ".diagnostics.rds"),
                        start_max_attempts = 50L) {
  ...
  write.csv(fit$diagnostics$comparison, paste0(out_csv, ".stages.csv"), row.names = FALSE)
  beta_hat <- fit$beta
  W_current <- fit$W
  beta_best_val <- fit$objective
  beta_conv_flag <- fit$beta_convergence
  # inference on beta
  ...
  out <- data.frame(
    ...
    outer_converged = fit$diagnostics$outer_converged,
    final_loop = fit$diagnostics$selected_stage1_iterations,
    cb_converged = fit$diagnostics$stage2$converged,
...
run_one_sim(seed = 1, out_csv = "single_replication/inference_seed1.csv",
            true_beta = true_beta, n_starts = 20, loops = 20,
            start_lower = true_beta - 2, start_upper = true_beta + 2)
```
