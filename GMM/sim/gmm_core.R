...
run_start_point <- function(seed, loop, start_id, base_beta, out_csv,
  ...
  fit <- tryCatch({
    ...
    if (loop == 1) {
      ...
      # Later loops are single-start refinements from this task's previous
      # optimized beta; do not add another random perturbation.
      stpt <- base_beta
    }

    optim(stpt, gmm_obj, sim_dat = sim_dat, W_hat = W_current,
          Time = Time, control = list(maxit = maxit))
  }, error = function(e) e)
...
best_beta_from_loop_file <- function(loop_csv, require_converged = TRUE) {
  ...
  if (!all(beta_cols %in% names(x))) {
    stop("Loop CSV is missing beta columns: ", loop_csv)
  }
  beta_matrix <- as.matrix(x[beta_cols])
  x <- x[is.finite(x$value) & apply(is.finite(beta_matrix), 1, all), , drop = FALSE]
  if (require_converged) {
    x <- x[x$convergence == 0, , drop = FALSE]
  }
  if (!nrow(x)) stop("No usable rows in ", loop_csv)
  as.numeric(unlist(x[which.min(x$value), beta_cols], use.names = FALSE))
}
