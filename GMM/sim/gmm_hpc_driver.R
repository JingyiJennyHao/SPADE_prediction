...
n_loops <- as.integer(get_arg("loops", "6"))
n_starts <- as.integer(get_arg("starts", "10"))
target <- as.integer(get_arg("target", "2"))
target_type <- get_arg("target-type", "converged")
...
if (n_starts < 1) stop("--starts must be positive")
if (target < 1) stop("--target must be positive")
if (target > n_starts) stop("--target cannot be larger than --starts")
if (J < 1) stop("--J must be positive")
if (maxit < 1) stop("--maxit must be positive")
if (!is.finite(inference_prob) || inference_prob <= 0 || inference_prob >= 1) {
  stop("--inference-prob must be between 0 and 1")
}
if (!target_type %in% c("completed", "converged")) {
  stop("--target-type must be either 'completed' or 'converged'")
}
...
loop_counts <- function(loop_csv) {
  if (!file.exists(loop_csv)) return(list(total = 0L, converged = 0L, usable = 0L))
  x <- tryCatch(read.csv(loop_csv, stringsAsFactors = FALSE), error = function(e) NULL)
  if (is.null(x) || !nrow(x)) return(list(total = 0L, converged = 0L, usable = 0L))
  beta_cols <- paste0("b", seq_along(true_beta))
  usable <- if (all(beta_cols %in% names(x)) && "value" %in% names(x)) {
    beta_matrix <- as.matrix(x[beta_cols])
    sum(is.finite(x$value) & apply(is.finite(beta_matrix), 1, all))
  } else {
    0L
  }
  list(
...
target_count <- function(loop, counts) {
  if (loop == 1L) {
    if (identical(target_type, "converged")) counts$converged else counts$total
  } else {
    counts$usable
  }
}

# Loop 1 explores multiple random starts. Every later loop is a single
# optimization initialized exactly at the beta selected by the prior loop.
starts_for_loop <- function(loop) if (loop == 1L) n_starts else 1L
target_for_loop <- function(loop) if (loop == 1L) target else 1L

summarize_loop <- function(loop, loop_csv, previous_beta, job_id = NA_character_,
                           reason = "completed") {
  counts <- loop_counts(loop_csv)
  require_converged <- loop == 1L && require_converged_best
  best_beta <- best_beta_from_loop_file(loop_csv, require_converged = require_converged)
  beta_diff <- max(abs(best_beta - previous_beta))

  loop_rows <- read.csv(loop_csv, stringsAsFactors = FALSE)
  beta_cols <- paste0("b", seq_along(true_beta))
  beta_matrix <- as.matrix(loop_rows[beta_cols])
  usable_row <- is.finite(loop_rows$value) &
    apply(is.finite(beta_matrix), 1, all)
  candidate_rows <- loop_rows[usable_row, , drop = FALSE]
  ...
  row <- data.frame(
    ...
    loop = loop,
    job_id = job_id,
    requested_starts = starts_for_loop(loop),
    target_results = target_for_loop(loop),
    target_type = if (loop == 1L) target_type else "usable_beta",
    completed_rows = counts$total,
    converged_rows = counts$converged,
    usable_beta_rows = counts$usable,
    require_converged_best = as.integer(require_converged),
...
submit_loop <- function(loop, base_beta_file, loop_csv) {
  n_loop_starts <- starts_for_loop(loop)
  job_name <- if (loop == 1L) {
    sprintf("gmm_s%d_l%d[1-%d]", seed, loop, n_loop_starts)
  } else {
    sprintf("gmm_s%d_l%d", seed, loop)
  }
...
if (start_loop > 1) {
  for (prior_loop in seq_len(start_loop - 1)) {
    ...
    log_msg(sprintf(
      "Existing loop %d check: completed=%d, converged=%d, usable_beta=%d, next_base_exists=%s",
      prior_loop, counts$total, counts$converged, counts$usable, file.exists(next_base)
    ))

    if (!file.exists(next_base)) {
      prior_usable <- if (prior_loop == 1L && require_converged_best) {
        counts$converged
      } else {
        counts$usable
      }
      if (prior_usable == 0) {
        stop("Cannot start from loop ", start_loop,
             ": missing ", next_base, " and no usable beta rows in ", prior_csv)
      }
      ...
      log_msg(sprintf(
        "Loop %d already got %d completed rows, %d converged rows, and %d usable beta rows; wrote %s.",
        prior_loop, summary$counts$total, summary$counts$converged,
        summary$counts$usable, next_base
      ))
    } else {
...
log_msg(sprintf(
  "Starting driver at loop %d/%d; loop 1 target=%d %s rows from %d starts; later loops use 1 usable beta per loop.",
  start_loop, n_loops, target, target_type, n_starts
))

for (loop in start_loop:n_loops) {
  n_loop_starts <- starts_for_loop(loop)
  loop_target <- target_for_loop(loop)
  loop_csv <- loop_csv_file(loop)
  counts <- loop_counts(loop_csv)

  if (counts$total > 0L) {
    log_msg(sprintf(
      "Loop %d existing result file found: completed=%d, converged=%d, usable_beta=%d.",
      loop, counts$total, counts$converged, counts$usable
    ))
  }
  ...
  if (counts$total == 0L) {
    ...
    repeat {
      ...
      target_label <- if (loop == 1L) target_type else "usable_beta"
      log_msg(sprintf(
        "loop=%d completed=%d converged=%d usable_beta=%d target=%d/%d starts=%d active=%s elapsed=%.2fh",
        loop, counts$total, counts$converged, counts$usable, target_count(loop, counts),
        loop_target, n_loop_starts, active, elapsed
      ))

      if (target_count(loop, counts) >= loop_target) break
      if (!active) {
        stop("Loop ", loop, " finished with ", target_count(loop, counts), " ", target_label,
             " rows, below required target ", loop_target,
             ". No next loop will be started.")
      }
      if (elapsed > max_wait_hours) {
        stop("Timed out waiting for loop ", loop, " to reach ", loop_target,
             " ", target_label, " rows or finish all jobs.")
      }
      Sys.sleep(poll_seconds)
    }
  } else {
    if (target_count(loop, counts) >= loop_target) {
      log_msg(sprintf(
        "Loop %d already got %d usable target rows, at least target %d; start from next loop after summarizing.",
        loop, target_count(loop, counts), loop_target
      ))
    } else {
      stop("Existing results for loop ", loop, " contain ", target_count(loop, counts),
           " usable target rows, below required target ", loop_target,
           ". Remove/repair the partial results or resume the missing jobs before continuing.")
    }
  }

  if (stop_remaining && loop == 1L && !is.na(job_id)) {
  ...
  counts <- loop_counts(loop_csv)
  require_converged <- loop == 1L && require_converged_best
  if (require_converged && counts$converged == 0L) {
    stop("Loop ", loop, " has no converged rows to choose next beta from: ", loop_csv)
  }
  if (!require_converged && counts$usable == 0L) {
    stop("Loop ", loop, " has no usable beta rows to choose next beta from: ", loop_csv)
  }
...
if (run_inference) {
  ...
  log_msg("Starting inference with the final selected beta and J=", J, ".")
  run_gmm_inference(
    beta_hat = base_beta,
    seed = seed,
  ...
  log_msg("Inference complete: ", inference_file)
}
