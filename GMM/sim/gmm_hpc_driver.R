...
submit_loop <- function(loop, base_beta_file, loop_csv) {
  ...
  out_log <- file.path(logs_dir, sprintf("loop%d_%%J_%%I.out", loop))
  err_log <- file.path(logs_dir, sprintf("loop%d_%%J_%%I.err", loop))
  ...
  cat("Submitting loop ", loop, ": ", cmd, "\n", sep = "")
  submit_out <- system(cmd, intern = TRUE)
  cat(paste(submit_out, collapse = "\n"), "\n")
...
for (loop in seq_len(n_loops)) {
  ...
  repeat {
    ...
    cat(sprintf("[%s] loop=%d converged=%d/%d elapsed=%.2fh\n",
                format(Sys.time(), "%F %T"), loop, done, target, elapsed))
  ...
  if (stop_remaining && !is.na(job_id)) {
    system(paste("bkill", shQuote(job_id)))
  }
  ...
  cat(sprintf("Loop %d complete: best_start=%s best_value=%.8g beta_diff=%.8g\n",
              loop, best_start_id, best_value, beta_diff))
}

cat("All requested loops complete.\n")
cat("Summary: ", summary_file, "\n", sep = "")
cat("Final beta: ", base_beta_file, "\n", sep = "")
