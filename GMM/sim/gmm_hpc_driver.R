...
n_starts <- as.integer(get_arg("starts", "50"))
target <- as.integer(get_arg("target", "20"))
...
submit_loop <- function(loop, base_beta_file, loop_csv) {
  ...
  job_name <- sprintf("gmm_s%d_l%d[1-%d]", seed, loop, n_loop_starts)
...
log_msg(sprintf(
  "Starting driver at loop %d/%d; loop 1 target=%d %s rows from %d starts; later loops use 1 start.",
...
for (loop in start_loop:n_loops) {
  ...
  if (counts$total == 0L) {
    ...
    repeat {
      ...
      log_msg(sprintf(
        "loop=%d completed=%d converged=%d target_%s=%d/%d starts=%d active=%s elapsed=%.2fh",
        loop, counts$total, counts$converged, target_type, target_count(counts),
        loop_target, n_loop_starts, active, elapsed
      ))

      if (target_count(counts) >= loop_target) break
      if (!active) {
        stop("Loop ", loop, " finished with ", target_count(counts), " ",
             target_type, " rows, below required target ", loop_target,
             ". No next loop will be started.")
      }
      if (elapsed > max_wait_hours) {
        stop("Timed out waiting for loop ", loop, " to reach ", loop_target,
             " ", target_type, " rows or finish all jobs.")
      }
      Sys.sleep(poll_seconds)
    }
  } else {
    if (target_count(counts) >= loop_target) {
      log_msg(sprintf(
        "Loop %d already got %d %s results, at least target %d; start from next loop after summarizing.",
        loop, target_count(counts), target_type, loop_target
      ))
    } else {
      stop("Existing results for loop ", loop, " contain ", target_count(counts),
           " ", target_type, " rows, below required target ", loop_target,
