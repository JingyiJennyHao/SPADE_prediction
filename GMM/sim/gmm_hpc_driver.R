...
target <- as.integer(get_arg("target", "1"))
...
log_msg(sprintf(
  "Starting driver at loop %d/%d with target=%d %s rows, starts=%d.",
...
for (loop in start_loop:n_loops) {
  ...
  if (counts$total == 0L) {
    ...
    repeat {
      ...
      log_msg(sprintf(
        "loop=%d completed=%d converged=%d target_%s=%d/%d active=%s elapsed=%.2fh",
        loop, counts$total, counts$converged, target_type, target_count(counts), target, active, elapsed
      ))

      if (target_count(counts) >= target) break
      if (!active) {
        stop_reason <- "job_finished_before_target"
        log_msg(sprintf(
          "Loop %d job %s is no longer active with completed=%d and converged=%d; proceeding.",
          loop, job_id, counts$total, counts$converged
        ))
        break
      }
    ...
    if (target_count(counts) >= target) {
      log_msg(sprintf(
        "Loop %d already got %d %s results, at least target %d; start from next loop after summarizing.",
        loop, target_count(counts), target_type, target
      ))
    } else {
      stop_reason <- "existing_results_below_target"
      log_msg(sprintf(
        "Loop %d has completed=%d and converged=%d below target %d %s rows and no active job is tracked; treating as no jobs left and proceeding.",
        loop, counts$total, counts$converged, target, target_type
      ))
    }
  }
