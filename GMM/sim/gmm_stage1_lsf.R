...
gmm_lsf_array <- function(payload, n_jobs, worker, label, output_prefix,
  ...
  if (!all(file.exists(outputs))) {
    stop(label, " jobs ended without result files for indices: ",
         paste(which(!file.exists(outputs)), collapse = ", "),
         ". Inspect job logs in ", run_dir)
  }
  lapply(outputs, readRDS)
}
