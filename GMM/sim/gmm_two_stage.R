...
gmm_refine <- function(beta_init, objective, max_iter = 20L, tol = 1e-5,
  ...
  for (iter in seq_len(max_iter)) {
    ...
    converged <- iter > 1L && is.finite(c_change) && c_change < tol
    beta <- beta_new
    cc <- c_new
    if (converged) break
  }
...
gmm_two_stage <- function(sim_dat, true_beta, n_starts = 20L, loops = 20L,
                          ...
                          diagnostics_file = NULL, start_max_attempts = 50L,
                          on_stage_selected = NULL) {
  stopifnot(length(true_beta) == 14L, all(is.finite(true_beta)))
  for (x in list(n_starts, loops, maxit, cb_max_iter, cb_beta_maxit, start_max_attempts))
  ...
  diagnostics <- list(settings = list(true_beta = true_beta, n_starts = n_starts,
    ...
    start_sd = start_sd, start_max_attempts = start_max_attempts, maxit = maxit, outer_tol = outer_tol,
    cb_max_iter = cb_max_iter, cb_tol = cb_tol, c_interval = c_interval,
    cb_beta_maxit = cb_beta_maxit,
    workflow = "identity_W_stage1_then_c_converged_stage2_dual_inference"))
  checkpoint <- function() {
  ...
  t1 <- proc.time()[[3L]]
  candidate_pools <- lapply(seq_len(n_starts), function(id) {
    lapply(seq_len(start_max_attempts), function(attempt) {
      if (uniform) runif(14, start_lower, start_upper) else
        true_beta + rnorm(14, 0, start_sd)
    })
  })
  initials <- lapply(candidate_pools, `[[`, 1L) # First proposals, not accepted points.
  diagnostics$initials <- initials
  diagnostics$candidate_pools <- candidate_pools
  checkpoint()
  if (identical(Sys.getenv("GMM_STAGE1_BACKEND"), "lsf")) {
    starts <- gmm_stage1_lsf(initials, sim_dat, true_beta, maxit,
                             loops, outer_tol, diagnostics_file, candidate_pools, diagnostics$settings)
  } else {
  ...
  checkpoint()
  if (!length(valid)) {
    diagnostics$stage1 <- list(status = "failed", error = "No finite converged Stage 1 paths.",
                              inference = list(status = "not_run"))
    diagnostics$stage2 <- list(status = "not_run", inference = list(status = "not_run"))
    checkpoint()
    stop("No Stage 1 paths met outer_tol; inspect diagnostics (no unconverged fallback).")
  }
  ...
  diagnostics$stage1$optim_convergence <- starts[[ordered[1]]]$fit$convergence
  diagnostics$stage1$converged <- TRUE
  diagnostics$stage1$stage1_iterations <- starts[[ordered[1]]]$iterations
  finish_stage <- function(label) {
    stage <- diagnostics[[label]]
    stage$status <- "selected"
    # Recompute W from this stage's final effective estimate, before inference.
    evaluation <- tryCatch({
      W <- W_hat_fun(stage$beta, sim_dat, Time = 3)
      value <- gmm_obj(stage$beta, sim_dat, W_hat = W, Time = 3)
      if (any(!is.finite(W)) || length(value) != 1L || !is.finite(value))
        stop("Nonfinite final W or objective")
      list(W = W, dynamic_objective = value)
    }, error = function(e) list(error = conditionMessage(e)))
    if (!is.null(evaluation$error)) {
      stage$inference <- list(status = "failed", error = evaluation$error)
      stage$dynamic_objective <- NA_real_
    } else {
      stage$W_final <- evaluation$W
      stage$dynamic_objective <- evaluation$dynamic_objective
    }
    diagnostics[[label]] <<- stage
    checkpoint()
    if (is.null(stage$inference)) {
      stage$inference <- if (is.null(on_stage_selected)) list(status = "not_requested") else
        tryCatch(on_stage_selected(label, stage),
                 error = function(e) list(status = "failed", error = conditionMessage(e)))
      diagnostics[[label]] <<- stage
      checkpoint()
    }
    if (identical(stage$inference$status, "failed"))
      cat(sprintf("%s inference failed: %s\n", label, stage$inference$error))
  }
  comparison <- function() {
    rows <- lapply(c("stage1", "stage2"), function(label) {
      stage <- diagnostics[[label]]
      selected <- identical(stage$status, "selected")
      row <- data.frame(stage = label, status = stage$status,
        error = if (is.null(stage$error)) "" else stage$error,
        start_id = if (selected) stage$start_id else NA_integer_,
        objective = if (selected) stage$objective else NA_real_,
        dynamic_objective = if (selected) stage$dynamic_objective else NA_real_,
        objective_definition = if (selected) stage$objective_definition else "",
        iterations = if (selected) stage$iterations else NA_integer_,
        seconds = stage$seconds, inference_status = stage$inference$status,
        inference_error = if (is.null(stage$inference$error)) "" else stage$inference$error,
        n_submitted = n_starts, n_received = diagnostics$n_received,
      ...
      beta <- if (selected) stage$beta else rep(NA_real_, 14)
      row[paste0("b", 1:14)] <- as.list(beta)
      row[paste0("error_b", 1:14)] <- as.list(beta - true_beta)
      row$l2_error <- sqrt(sum((beta - true_beta)^2))
      row
    })
    do.call(rbind, rows)
  }
  finish_stage("stage1") # Persist its inference before dispatching Stage 2.
  diagnostics$stage2 <- list(status = "pending", seconds = NA_real_,
                            inference = list(status = "not_run"))
    ...
    if (identical(Sys.getenv("GMM_STAGE1_BACKEND"), "lsf")) {
      gmm_lsf_array(
        ...
        validate_result = function(result, id) {
          stopifnot(is.list(result), identical(as.integer(result$start_id), as.integer(top[id])))
          if (is.null(result$error)) stopifnot(
            is.numeric(result$beta_effective), length(result$beta_effective) == 14L,
            is.numeric(result$objective), length(result$objective) == 1L,
            is.logical(result$converged), length(result$converged) == 1L)
        },
        failed_result = function(id, reason) list(start_id = top[id],
          original_objective = starts[[top[id]]]$fit$value,
          converged = FALSE, error = reason))
    } else {
      lapply(starts[top], function(candidate) {
  ...
  eligible <- which(vapply(refined, function(s) is.null(s$error) &&
    isTRUE(s$converged) && length(s$objective) == 1L && is.finite(s$objective) &&
    length(s$beta_effective) == 14L && all(is.finite(s$beta_effective)), logical(1)))
  diagnostics$stage2_eligible_ranks <- eligible
  if (!length(eligible)) {
    reason <- if (!is.null(dispatch_error)) dispatch_error else
      "No finite Stage 2 candidate met the c convergence criterion."
    diagnostics$stage2 <- list(status = "failed", error = reason,
      seconds = proc.time()[[3L]] - t2, inference = list(status = "not_run"))
    cat("Stage 2 failed; Stage 1 results retained: ", reason, "\n", sep = "")
  } else {
    pick <- eligible[which.min(vapply(refined[eligible], function(s) s$objective, numeric(1)))]
    selected <- refined[[pick]]
    diagnostics$selected_candidate_rank <- pick
    diagnostics$stage2 <- list(start_id = selected$start_id, beta = selected$beta_effective,
      objective = selected$objective, seconds = proc.time()[[3L]] - t2,
      iterations = selected$iterations, converged = TRUE,
      optim_convergence = selected$optim_convergence,
      stage1_iterations = starts[[selected$start_id]]$iterations,
  ...
  diagnostics$comparison <- comparison()
  diagnostics$comparison$dynamic_objective_improvement <-
    diagnostics$comparison$dynamic_objective[1] - diagnostics$comparison$dynamic_objective[2]
  checkpoint()
  list(stage1 = diagnostics$stage1, stage2 = diagnostics$stage2, diagnostics = diagnostics)
