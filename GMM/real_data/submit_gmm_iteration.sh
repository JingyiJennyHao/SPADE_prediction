...
RESULTS_DIR="${RESULTS_DIR:-$PROJECT_DIR/results/manual_iteration}"
GROUP_COUNT="${GROUP_COUNT:-210}"
TOTAL_TASKS="${TOTAL_TASKS:-120}"
JOB_NAME="${JOB_NAME:-gmm_iter}"
...
BETA0_FILE="${BETA0_FILE:?BETA0_FILE is required. Point it to the initial beta or previous GMM summary RDS.}"
...
SEED=\$((700000 + TASK_ID))
Rscript gmm_start_runner.R \
  --groups "${GROUP_COUNT}" \
  --task-id "\${TASK_ID}" \
  --seed "\${SEED}" \
  --data "${DATA_PATH}" \
  --beta0-file "${BETA0_FILE}" \
  --out "${RESULTS_DIR}/runs/task_\$(printf "%03d" "\${TASK_ID}").rds" \
  --start-scale "${START_SCALE}" \
  --ridge "${RIDGE}" \
  --maxit "${MAXIT}" \
  --uncentered-v "${UNCENTERED_V}"
EOF

echo "Submitted GMM iteration batch."
echo "Results directory: ${RESULTS_DIR}"
echo "Shared log: ${RESULTS_DIR}/logs/${JOB_NAME}.out"
echo "Shared err: ${RESULTS_DIR}/logs/${JOB_NAME}.err"
echo "After enough tasks finish, run:"
echo "cd ${PROJECT_DIR}"
echo "module load ${R_MODULE}"
echo "export R_LIBS_USER=\"${R_LIBS_USER_PATH}\""
echo "Rscript collect_gmm_results.R --input-dir ${RESULTS_DIR}/runs --out-csv ${RESULTS_DIR}/summary/${JOB_NAME}.csv"
echo "Rscript gmm_summarize.R --input-dir ${RESULTS_DIR}/runs --out ${RESULTS_DIR}/summary/beta_hat.rds"
